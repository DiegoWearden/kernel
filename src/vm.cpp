...
static inline uint64_t get_memory_attributes(uint64_t phys_addr) {
    if (phys_addr >= 0x3F000000) { // perihperals + arm device memory (0x3F000000 - 0x40000000 and 0x40000000+)
...
void create_page_tables() {
    ...
    // Map low address space (0x0 + PAGE_SIZE_2MB - 0x40000000) - identity mapping
